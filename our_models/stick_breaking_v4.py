...
class StickBreakingV4(torch.nn.Module):
    def __init__(self, k, kmer_dim=2, lr=0.1, epoch_num=100, batch_size=1000,
                 device=torch.device("cpu"), verbose=False, seed=0):

        super(StickBreakingV4, self).__init__()

        self.__seed = seed
        self.__k = k
        self.__kmer_dim = kmer_dim
        self.__lr = lr
        ...
        self.__kmer_embs = torch.nn.Parameter(
            2 * torch.rand(size=(4**self.__k, self.__kmer_dim), device=self.__device) - 1, requires_grad=True
        )
        self.__weights0 = torch.nn.Parameter(
            2 * torch.rand(size=((4**self.__k) * self.__kmer_dim, 512), device=self.__device) - 1, requires_grad=True
        )
    ...
    def single_pass(self, kmers_profile):
        ...
        kmers_profile = kmers_profile / kmers_profile.sum(dim=1).view(-1, 1)

        input = (kmers_profile.unsqueeze(2) * self.__kmer_embs.unsqueeze(0)).reshape(
            kmers_profile.shape[0], self.__kmer_embs.shape[0] * self.__kmer_embs.shape[1]
        )

        output0 = torch.sigmoid(input @ self.__weights0 + self.__bias0)
        output1 = torch.sigmoid(output0 @ self.__weights1 + self.__bias1)
        output2 = output1 @ self.__weights2

        # Standardize the output
        output5 = output2 #(output2 - output2.mean(dim=0)) / output2.std(dim=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--kmer_dim', type=int, default=2, help='Dimension of k-mer embedding')
    ...
    model = StickBreakingV4(
        k=args.k, kmer_dim=args.kmer_dim,
    ...
    new_model = StickBreakingV4(**kwargs)
